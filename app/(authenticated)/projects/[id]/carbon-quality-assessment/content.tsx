...
export function Content({
  ...
  if (!projectData || !projectData.carbonQualityAssessment) {
    return (
      <div className="w-full p-4 bg-background rounded-sm border mr-2 flex items-center justify-center">
        <span>{t('carbonQualityAssessment.title')} data is not available for this project.</span>
      </div>
    );
  }

  const carbonQualityAssessment = projectData.carbonQualityAssessment;
  return (
    <div className="w-full p-4 bg-background rounded-sm border mr-2 overflow-scroll">
      <div className="p-4">
        <span className="text-lg font-semibold">{t('carbonQualityAssessment.categories')}</span>
      </div>
      <Table>
        <TableHeader>
          <TableRow>
            <TableHead>{t('carbonQualityAssessment.name')}</TableHead>
            <TableHead>{t('carbonQualityAssessment.qatalystResponses')}</TableHead>
            <TableHead>{t('carbonQualityAssessment.userRating')}</TableHead>
            <TableHead>{t('common.source')}</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {carbonQualityAssessment.risks?.map((risk) => {
            return (
              <TableRow
                key={risk.id}
