...
export function Content({
  ...
  if (!projectData || (!projectData.permanence && !projectData.carbonQualityAssessment)) {
    return (
      <div className="w-full p-4 bg-background rounded-sm border mr-2 flex items-center justify-center">
        <span>{t('permanence.title')} data is not available for this project.</span>
      </div>
    );
  }

  // Use permanence if available, otherwise fallback to carbonQualityAssessment
  const permanence = projectData.permanence || projectData.carbonQualityAssessment;
  return (
    <div className="w-full p-4 bg-background rounded-sm border mr-2 overflow-scroll">
      <div className="p-4">
        <span className="text-lg font-semibold">{t('permanence.categories')}</span>
      </div>
      <Table>
        <TableHeader>
          <TableRow>
            <TableHead>{t('permanence.name')}</TableHead>
            <TableHead>{t('permanence.qatalystResponses')}</TableHead>
            <TableHead>{t('permanence.userRating')}</TableHead>
            <TableHead>{t('common.source')}</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {permanence?.risks?.filter(risk => 
