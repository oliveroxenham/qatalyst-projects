...
export function Content({
  ...
  if (!projectData || (!projectData.additionality && !projectData.carbonQualityAssessment)) {
    return (
      <div className="w-full p-4 bg-background rounded-sm border mr-2 flex items-center justify-center">
        <span>{t('additionality.title')} data is not available for this project.</span>
      </div>
    );
  }

  // Use additionality if available, otherwise fallback to carbonQualityAssessment
  const additionality = projectData.additionality || projectData.carbonQualityAssessment;
  return (
    <div className="w-full p-4 bg-background rounded-sm border mr-2 overflow-scroll">
      <div className="p-4">
        <span className="text-lg font-semibold">{t('additionality.categories')}</span>
      </div>
      <Table>
        <TableHeader>
          <TableRow>
            <TableHead>{t('additionality.name')}</TableHead>
            <TableHead>{t('additionality.qatalystResponses')}</TableHead>
            <TableHead>{t('additionality.userRating')}</TableHead>
            <TableHead>{t('common.source')}</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {additionality?.risks?.filter(risk => 
