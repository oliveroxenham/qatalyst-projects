...
export function Content({
  ...
  if (!projectData || (!projectData.reputationalRisk && !projectData.carbonQualityAssessment)) {
    return (
      <div className="w-full p-4 bg-background rounded-sm border mr-2 flex items-center justify-center">
        <span>{t('reputationalRisk.title')} data is not available for this project.</span>
      </div>
    );
  }

  // Use reputationalRisk if available, otherwise fallback to carbonQualityAssessment
  const reputationalRisk = projectData.reputationalRisk || projectData.carbonQualityAssessment;
  return (
    <div className="w-full p-4 bg-background rounded-sm border mr-2 overflow-scroll">
      <div className="p-4">
        <span className="text-lg font-semibold">{t('reputationalRisk.categories')}</span>
      </div>
      <Table>
        <TableHeader>
          <TableRow>
            <TableHead>{t('reputationalRisk.name')}</TableHead>
            <TableHead>{t('reputationalRisk.qatalystResponses')}</TableHead>
            <TableHead>{t('reputationalRisk.userRating')}</TableHead>
            <TableHead>{t('common.source')}</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {reputationalRisk?.risks?.filter(risk => 
            !risk.name.startsWith('Risk 1:') && 
            !risk.name.startsWith('Risk 2:') && 
